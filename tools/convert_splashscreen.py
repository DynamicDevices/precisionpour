#!/usr/bin/env python3
"""
Convert Precision Pour splashscreen image to LVGL C array format (RGB565)
"""
...
def convert_to_lvgl_rgb565(input_path, output_path, var_name="precision_pour_img"):
    ...
    print(f"Converting image: {width}x{height} pixels")
    ...
    with open(output_path, 'w') as f:
        f.write(f"/**\n")
        f.write(f" * Precision Pour Splashscreen Image\n")
        ...
        f.write(f"    .header = {{\n")
        f.write(f"        .cf = LV_IMG_CF_TRUE_COLOR,  // RGB565 format\n")
        f.write(f"        .flags = 0,\n")
        f.write(f"        .w = {width},\n")
        f.write(f"        .h = {height},\n")
        f.write(f"    }},\n")
...
if __name__ == "__main__":
    ...
    input_image = os.path.join(project_root, "resources", "precision_pour_resized.png")
    output_header = os.path.join(project_root, "include", "images", "precision_pour_img.h")
