...
def extract_logo(input_path, output_path, var_name="precision_pour_logo"):
    ...
    Args:
        input_path: Path to input full-screen image
        output_path: Path to output logo image
        var_name: Variable name (unused, kept for compatibility)
    
    Returns:
        tuple: (width, height) of the extracted logo
    """
    try:
        # Open original image
        ...
        print(f"Error extracting logo: {e}", file=sys.stderr)
        sys.exit(1)
