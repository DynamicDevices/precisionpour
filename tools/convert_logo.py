...
def convert_to_lvgl_rgb565(input_path, output_path, var_name="precision_pour_logo"):
    ...
    print(f"  Original splashscreen: 153600 bytes")
    print(f"  Logo size: {len(rgb565_data)} bytes")
    print(f"  Space saved: {153600 - len(rgb565_data)} bytes ({((153600 - len(rgb565_data)) / 153600 * 100):.1f}%)")
    return width, height
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_image):
        ...
        print(f"Run extract_logo.py first to create the logo image")
        sys.exit(1)
